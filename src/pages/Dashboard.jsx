      ...
      try {
        const [created, joined] = await Promise.all([
          ...
          supabase
            ...
            .select('session:sessions(*, session_participants(user_id))')
            .eq('user_id', user.id),
        ])
        ...
        setJoinedSessions((joined.data || []).map(r => r.session).filter(Boolean))
      } catch (err) {
        setCreatedSessions([])
        setJoinedSessions([])
        if (isMissingSupabaseSchema(err)) {
          setWarning('Session-Daten sind noch nicht vollständig eingerichtet.')
          console.warn('Dashboard-Sessiondaten fehlen:', err)
        } else {
          setWarning('Sessions konnten gerade nicht geladen werden.')
          console.warn('Dashboard konnte Sessions nicht laden:', err)
        }
      }
      setLoading(false)
    }
            ...
            <>
              ...
              {upcomingJoined.length > 0 && (
                <>
                  ...
                  {upcomingJoined.map(s => <SessionRow key={s.id} session={s} />)}
                </>
              )}
            </>
          )}
      ...
      {tab === 'joined' && (
        <div className="flex flex-col gap-3">
          {joinedSessions.length === 0
            ...
            : joinedSessions.map(s => s && <SessionRow key={s.id} session={s} />)
          }
        </div>
      )}
